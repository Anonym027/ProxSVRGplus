...
def main():
    ...
    # Set the outer batch size B = n/5, as suggested in the paper.
    B = n // 5
    print(f"Use outer batch size B = n/5 = {B}")

    # A grid of inner batch sizes (b) to sweep through for the experiment.
    b_list = [1, 4, 16, 64, 256, 512, 1024, 2048, 4096, 8192, 16384]

    # Run for 6 epochs to generate plots where the x-axis (#SFO/n) reaches ~6.
    max_epochs = 6

    for b in b_list:
        print("\n" + "=" * 70)
        print(f"[a9a] Running all algorithms with minibatch b = {b}")
        print("=" * 70)

        # ===== Step sizes based on the paper's formulas =====
        eta_gd = 1.0 / L
        eta_sgd = 1.0 / (2.0 * L)
        ...
        out_gd = results_dir / f"results_a9a_b{b}_proxgd.npy"
        ...
        out_sgd = results_dir / f"results_a9a_b{b}_proxsgd.npy"
        np.save(out_sgd, hist_sgd.as_dict())
        print("  saved to:", out_sgd)

        # ===== 3) Run ProxSVRG =====
        print("\n[ProxSVRG]")
        # Number of inner iterations (m), a common choice is n/b.
        inner_iters = n//b
        ...
        out_svrg = results_dir / f"results_a9a_b{b}_proxsvrg.npy"
        ...
        out_plus = results_dir / f"results_a9a_b{b}_proxsvrgplus.npy"
